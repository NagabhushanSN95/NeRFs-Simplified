...
# Loads NeRF Synthetic Data for NeRF
# Modified from NerfLLffDataLoader01.py.
...
class NerfSyntheticDataLoader(DataLoaderParent):
    def __init__(self, configs: dict, data_dirpath: Path, mode: Optional[str]):
        super(NerfSyntheticDataLoader, self).__init__()
        self.configs = configs
        self.data_dirpath = data_dirpath
        self.mode = mode
        self.scene_name = configs['data_loader']['scene_id']
        return
    ...
    def load_nerf_data(self, data_dict):
        ...
        images_dirpath = self.data_dirpath / f'all/DatabaseData/{self.scene_name}/{self.mode}/rgb'
        ...
        bounds = numpy.array([2, 6]).astype('float32')

        extrinsics_path = self.data_dirpath / f'all/DatabaseData/{self.scene_name}/{self.mode}/CameraExtrinsics.csv'
        extrinsic_matrices = numpy.loadtxt(extrinsics_path.as_posix(), delimiter=',').reshape((-1, 4, 4))
        poses = extrinsic_matrices[frame_nums]

        intrinsics_path = self.data_dirpath / f'all/DatabaseData/{self.scene_name}/{self.mode}/CameraIntrinsics.csv'
